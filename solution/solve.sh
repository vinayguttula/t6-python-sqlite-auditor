...
def run_audit():
    ...
    with open(metrics_path, 'r') as f:
        ...
        for row in reader:
            metrics[row['endpoint']] = int(row['requests_last_24h'])
    ...
    for api_id, endpoint, approved_by in c.fetchall():
        if api_approver not in approved_by:
            if metrics.get(endpoint, 0) > 0:
    ...
    for (test_name,) in c.fetchall():
        service = test_mapping.get(test_name)
        tier = ownership.get("services", {}).get(service, {}).get("tier", 99) if service else 99
    ...
    c.execute("""
        SELECT t.id, t.summary, t.rollback_plan, e.status 
        FROM tickets t 
        LEFT JOIN exceptions e ON t.linked_ticket = e.id 
        WHERE t.type = 'Migration'
    """)
    for tkt_id, summary, rollback_plan, exc_status in c.fetchall():
        if not rollback_plan.strip() and migration_tag not in summary and exc_status != 'APPROVED':
            report["blockers"].append({
                "id": tkt_id,
                "type": "MIGRATION",
                "severity": "CRITICAL",
                "description": f"Migration ticket {tkt_id} missing rollback plan and lacks exception.",
